    ...
    $.when(
        ...
        echart_indus.setOption({
            ...
            title : {
                ...
                color:['palevioletred','pink','brown','amber','plum'],
                legend: {
                    type: 'scroll',
                    orient: 'vertical',
                    right: 50,
        ...
        echart_indus.hideLoading();       
        console.log("we failed to get the data");　　　　
    });
...
function setNews(){
    ...
    var scrollContHeight = 488; 
    var scrollContWidth = 440; 
    var scrollSpeed = 80; 
    ...
    $.when(
        $.ajax({
            ...
            success: function (dataresult) {
                ...
                if (dataresult) {
                    for (var i = 0; i < dataresult.length; i++) {
                        var inercov, inercov1, newstr
                        inercov = dataresult[i].key
                        inercov1 = inercov.charAt(inercov.length - 1);
                        (inercov1 == "市") ? newstr = inercov.substring(0, inercov.length - 1): newstr = inercov;
                        if (inercov1 != "省") {
                            data.push({
                                name: newstr,
                                value: dataresult[i].count
...
function btnrep(){
    ...
    console.log(urlpara);
    var urlajax1 = "http://10.20.118.28:9002/scope/"+urlpara; //table
    var urlajax2 = "http://10.20.118.28:9002/accum/"+urlpara; //chart
    ...
    //////////////
    var deftUrl = "http://10.20.118.28:9002/scope/"+urlpara;
    var tablesDataf=[];
